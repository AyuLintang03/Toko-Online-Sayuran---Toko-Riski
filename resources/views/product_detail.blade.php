...
<header class="header-section">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="product-pic-zoom">
                                <img class="product-big-img" src="{{url('storage/' . $product->image)}}" alt="" />
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="product-details">
                                <div class="pd-title">
                                    <span>{{$product->category_product->name_category_products}}</span>
                                    <h3>{{$product->name}}   <i class="fa fa-arrow-up" style="color: #ff0000;"></i></h3><br>
                                ...
                                <p>
                                  {{$product->description}}   
                                    ...
                                    <h4 class="product-price">Rp. {{$product->price}}/{{$product->satuan}}</h4><br>
                                    <form action="{{route('add_to_cart_product', $product->id)}}" method="post">
                                    @csrf
                                    <input type="number" name='amount' class="form-control" style="margin-left: -30px;" id="amountInput">
                                
                                </div>
                                 <div class="quantity">
                                    <button type="submit" class="primary-btn pd-cart">Tambahkan Keranjang</button>
                                </div>
                                </form>
                            </div>
                        </div>
                         ...
                         <div class="col-lg-6">
                            <div class="product-details">
                                <div class="pd-title">
                                    <span>{{$product->category_product->name_category_products}}</span>
                                    <h3>{{$product->name}}  <i class="fa fa-arrow-down" style="color: #A8DF8E;"></i></h3><br>
                                    <p>{{$product->jenis}}</p>
                                <p>Stock : {{$product->stock}}</p>
                                </div>
                                <div class="pd-desc">
    ...
    function showSweetAlertWithDelay() {
        ...
        setTimeout(function() {
            window.location.href = "{{ route('show_cart_product') }}";
        },  10); 
    }
</script>
