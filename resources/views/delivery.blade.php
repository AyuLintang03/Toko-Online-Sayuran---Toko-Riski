                    ...
                    @if ($order->status == 'Lunas')
                       @if ($order->delivery &&$order->delivery->delivery_status == 'Proses')
                        ...
                        @else
                            ...
                            @if ($order->delivery &&$order->delivery->delivery_status == 'Pesanan Diterima')
                              
                                 <p>Pesanan diterima</p> 
                            @elseif($order->delivery &&$order->delivery->delivery_status == 'Pesanan Sedang Diantar')
                            <p>Pesanan sedang diantar ke alamat anda, mohon tunggu sebentar</p>
                            <form action="{{ route('markOrderReceived', $order) }}" method="post">
                            @csrf
                            @method('PATCH')
                                  <button type="submit"class="btn btn-success">Pesanan Diterima</button></form>
                            @endif
                        @endif
                    @else
